...
// // Get movies with search and filters
// exports.getMovies = async (req, res) => {
//   const page = parseInt(req.query.page) || 1;
//   const limit = parseInt(req.query.limit) || 12;
//   const offset = (page - 1) * limit;
...
//     if (genres) {
//       whereClause = whereClause ? `${whereClause} AND genres @> ARRAY[$${paramCount}]::varchar[]` : `WHERE genres @> ARRAY[$${paramCount}]::varchar[]`;
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO movies 
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
        RETURNING *`,
      [title, description, poster_url, video_url, release_year, duration, genresArray, type]
    );
  ...
  try {
    ...
    const result = await query(
      `UPDATE movies 
        ...
        WHERE id = $9 
        RETURNING *`,
      [title, description, poster_url, video_url, release_year, duration, genresArray, type, id]
    );
